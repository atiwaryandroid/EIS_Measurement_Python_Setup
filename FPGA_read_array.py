...
def FPGA_read_array(xem):
    ...
    heatmap = 1 #0 to not call createHeatmap, 1 to call createHeatmap
    plot = 1 #0 not to plot graph for each of the sensors, 1 to do so
    ...
    for k in range(1, test_len + 1):
        ...
        for j in range(1, 21):     
            for m in range(1, no_col + 1):
                ...
                #data_transient[j - 1][m - 1][k - 1][0] = j + m
                if k == 1:
    ...
    if plot == 1:
        createPlots(data_transient, test_len, 20, no_col)
