...
def FPGA_read_teststr_loop(xem):
    ...
    for k in range(1, test_len + 1):
        ...
        buf = bytearray([0]) #Not really sure what buf does, but it seems to work
        temp_array1 = []
        for i in range(1, (4 * data_points) + 1):
            temp_array1.append(xem.ReadFromBlockPipeOut(0xb3, data_read_leng, buf)) #Not really sure what buf does, but it seems to work
